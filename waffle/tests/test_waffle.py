...
class WaffleTests(TestCase):
    ...
    def test_everyone_off(self):
        ...
        Flag.objects.create(name="myflag", everyone=False, authenticated=True)
    ...
    @override_settings(WAFFLE={"FLAG_DEFAULT": True, "FLAG_CLASS": "test_app.Flag"})
...
class SwitchTests(TestCase):
    ...
    def test_switch_active_from_cache(self):
        ...
        assert waffle.switch_is_active(switch.name)
        self.assertEqual(
            queries, len(connection.queries), "We should only make one query."
        )
